...
namespace Ekter
{
    Land::Land(GameDataRef data) : _data(data)
    {
        ...
        sprite.setPosition(0, 1.4 * (_data->window.getSize().y - sprite.getGlobalBounds().height));
        sprite2.setPosition(sprite.getGlobalBounds().width, 1.4 * (_data->window.getSize().y - sprite.getGlobalBounds().height));
